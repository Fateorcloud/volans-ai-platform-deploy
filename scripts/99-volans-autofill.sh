...
(function () {
  ...
  function lockInput(input) {
    ...
    input.setAttribute("data-volans-api-key", "locked");
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", hideApiKeyInputs);
  } else {
    hideApiKeyInputs();
  }

  new MutationObserver(hideApiKeyInputs).observe(document.documentElement, {
    childList: true,
    subtree: true
  });
})();
...
if [ -z "${IMAGE_SHARED_API_KEY:-}" ]; then
  echo "[volans] IMAGE_SHARED_API_KEY is empty; UI will still ask for a key"
else
  echo "[volans] image API key autofill enabled"
fi
